...
import { IXHoursAdayDoseTime } from '@/store/slices/features/medicineDetails/types';
...
const EveryXhoursDose: FC = () => {
  ...
  const handleNext: any = async () => {
    ...
    if (filterArray.length > 0) {
      ...
      if (loginStatus) {
        ...
        dispatch(setXhoursAdayStoreData(tempStore));
  ...
  useEffect(() => {
    if (times.every(time => time !== '') && doses.every(dose => dose !== 0)) {
      const fourTimesAdayDoses: IXHoursAdayDoseTime[] = times
        .map((time, index) => ({
          doseTime: time,
      ...
      dispatch(setXhoursAdayDoseTime(fourTimesAdayDoses));
    }
  }, [times, doses]);
