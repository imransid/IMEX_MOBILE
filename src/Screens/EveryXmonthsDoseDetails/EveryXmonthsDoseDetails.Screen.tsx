...
import { setWeeklyStoreData, setXMonthTakeDose } from '@/store/slices/features/medicineDetails/slice';
...
const EveryXmonthsDoseDetails: FC = () => {
  ...
  const xMonthTakeDoseTime = useSelector((state: RootState) => state.medicineDetails.xMonthTakeDoseTime);
  ...
  useEffect(() => {
    if (times.every(time => time !== '') && doses.every(dose => dose !== 0)) {
      ...
      dispatch(setXMonthTakeDose(weeklyDoses));
    }
  }, [times, doses]);
...
export default EveryXmonthsDoseDetails;
