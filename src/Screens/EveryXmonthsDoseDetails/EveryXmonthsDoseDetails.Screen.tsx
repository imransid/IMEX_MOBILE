...
import { IMonthlyDoseTime, IWeeklyDoseTime } from '@/store/slices/features/medicineDetails/types';
import {
  setWeeklyStoreData,
  setXmonthsTakeDose
...
const EveryXmonthsDoseDetails: FC = () => {
  ...
  const xMonthsTakeDoseTime = useSelector(
  ...
  const handleNext: any = () => {
    let filterArray = xMonthsTakeDoseTime.filter(e => {
  ...
  useEffect(() => {
    if (times.every(time => time !== '') && doses.every(dose => dose !== 0)) {
      ...
      dispatch(setXmonthsTakeDose(XDoses));
    }
  }, [times, doses]);
